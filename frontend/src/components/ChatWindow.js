...
const ChatWindow = () => {
  ...
  const handleGenerateReport = () => {
    console.log('Generating report...');
    endChat(); // End chat after report generation
  };

  const endChat = async () => {
    try {
      // Call the screening API to add a new screening
      await axios.post('/api/auth/addScreening', {
        student: studentId,
        volunteer: volunteerId,
        camp: campId,
        report: 'Sample report text here.' // Replace with actual report data if needed
      });

      // Navigate back to the previous page
      navigate(-1); // Navigate to the previous page
    } catch (error) {
      console.error('Error while ending chat:', error);
    }
  };
